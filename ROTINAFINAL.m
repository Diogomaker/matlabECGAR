...
mediapontoref = mean((batimentos(401,:))); 
% ponto máximo absoluto dos vetores batimentos é no indice 401

contador = [];
i = 0;
for ii = 1 : nbat1
    if abs(batimentos(401,ii)) < abs(mediapontoref*0.3) || abs(batimentos(401,ii)) > abs(mediapontoref*1.8)
        i = i + 1;
        contador(i) = ii;     
     end
end
...
% Voltar alguns pontos do pico do qrs e calcular o desvio padrao dos proximos
% 40 pontos, retirandoa a flutuação da linha de base com a função "detrend"

[~,index] = max(abs(batimentos(:,1)));
ruido_original = std(detrend(batimentos(index-121:index-81,1)));
%Ruido do sinal original

%% Detector de nível parte 1 (sem aplicar correlação)
...
% ruido final
[~,index] = max(abs(medio_dn));
ruido_final_dn = std(detrend(medio_dn(index-121:index-81,1)));

[~,index] = max(abs(medio_in));
ruido_final_in = std(detrend(medio_in(index-121:index-81,1)));

[~,index] = max(abs(medio_id));
ruido_final_id = std(detrend(medio_id(index-121:index-81,1)));
